...
function countAdjacentMines2(row, col) {
    //return pocet min, ktore su okolo danej pozicie
    return [-1, 0, 1].reduce((totalRow, r) => {
        return totalRow + [-1, 0, 1].reduce((totalCol, c) => {
            if (row + r >= 0 && row + r < rowCount
                && col + c >= 0 && col + c < colCount
                && field[row + r][col + c] instanceof Mine) {
                return totalCol++; // 1 mine to add
            } else {
                return totalCol; // there is no mine
            }
        }, 0) // 0 here - initial value, out of lambda fun
    }, 0);
}
...
function countAdjacentMines(row, col) {
    ...
    return [-1, 0, 1].reduce((totalRow, r) => {
        return totalRow + [-1, 0, 1].reduce((totalCol, c) => (
                (row + r >= 0 && row + r < rowCount
                && col + c >= 0 && col + c < colCount
                && field[row + r][col + c] instanceof Mine) ? totalCol++ : totalCol), 0) // 0 here - initial value, out of lambda fun
    }, 0);
}
// (acc, curr) => curr.state == StateEnum.OPEN ? acc : acc++
...
function isWon2() {
    // let numberOfOpenTiles = 0;    
    console.log("som tu");
    const numberOfOpenTiles = field.flat().reduce((acc, curr) => (curr.state == StateEnum.OPEN ? acc++ : acc ), 1);
    console.log(field);
    console.log(numberOfOpenTiles);
//     return [-1, 0, 1].reduce((totalRow, r) => {
//         return totalRow + [-1, 0, 1].reduce((totalCol, c) => (
//                 (row + r >= 0 && row + r < rowCount
//                 && col + c >= 0 && col + c < colCount
